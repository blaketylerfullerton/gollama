...
var aboutNotes = []string{
	...
	"go run ./cmd/inspect opens an interactive trace viewer: type a prompt, run it, then step between generated tokens to see what each one attended to and where its answer settled. The chat screen doesn't show this live, but every turn is recorded as it happens — \"past conversations\" on the welcome menu opens the same step-through over anything you've said to it.",
